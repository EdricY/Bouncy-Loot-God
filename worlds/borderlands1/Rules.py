...
def setup_level_rules(world: Borderlands1World):
...
def setup_custom_rules(world: Borderlands1World):
    ...
    if "Forge" not in world.restricted_regions:
        ...
        world.try_add_rule("Torgue Tokens Accessible", create_rule(world, BL1ArchiData("BadassCraterBar", 15), ""))

    world.try_add_rule("Seraph Crystals Accessible", 
        CanReachRegion("Sanctuary") # from black market
        | create_rule(world, BL1ArchiData("WashburneRefinery", 30, tags=["raidboss"], other_req_regions=["LeviathansLair"]), "") # hyperius
        | create_rule(world, BL1ArchiData("HaytersFolly", 30, tags=["raidboss"], other_req_regions=["LeviathansLair"]), "") # gee
        | create_rule(world, BL1ArchiData("PyroPetesBar", 30, req_rules=["Torgue DLC Complete"], tags=["raidboss"]), "") # pete
        | create_rule(world, BL1ArchiData("CandlerakksCrag", 30, tags=["raidboss"], other_req_regions=["Terminus"]), "") # voracidous
        | create_rule(world, BL1ArchiData("WingedStorm", 38, tags=["raidboss"]), "") # ancient dragons
        # | create_rule(world, BL1ArchiData("FlamerockRefuge", 30), "") # tina slot machine (insane currently)
    )

# creates a rule for a location, ignores location_data.alternates
def create_rule(world: Borderlands1World, location_data: BL1ArchiData, location_name: str, force_included=False):
    rule = True_()
